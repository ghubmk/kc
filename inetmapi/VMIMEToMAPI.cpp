		...
		if(vmHeader->hasField(vmime::fields::FROM)) {
			...
			if (hr == hrSuccess) {
				...
				hr = ECCreateOneOff(reinterpret_cast<const TCHAR *>(wstrFromName.c_str()),
				     ...
				     reinterpret_cast<const TCHAR *>(convert_to<std::wstring>(strFromEmail).c_str()),
		...
		if (vmHeader->hasField(vmime::fields::SENDER) || vmHeader->hasField(vmime::fields::FROM)) {
			...
			if (hr == hrSuccess) {
				...
				hr = ECCreateOneOff(reinterpret_cast<const TCHAR *>(wstrSenderName.c_str()),
				     reinterpret_cast<const TCHAR *>(L"SMTP"),
				     reinterpret_cast<const TCHAR *>(convert_to<std::wstring>(strSenderEmail).c_str()),
    ...
    vmime::shared_ptr<vmime::addressList> vmAddressList, ULONG ulRecipType)
{
	...
	for (int iRecip = 0; iRecip < iAddressCount; ++iRecip) {
		...
		hr = ECCreateOneOff(reinterpret_cast<const TCHAR *>(wstrName.c_str()),
		     reinterpret_cast<const TCHAR *>(L"SMTP"),
		     reinterpret_cast<const TCHAR *>(convert_to<std::wstring>(strEmail).c_str()),
    ...
    const std::vector<std::string> &cs)
{
	...
	for (size_t i = 0; i < cs.size(); ++i) {
		...
		try {
			data = convert_to<std::string>((cs[i] + "//NOIGNORE").c_str(),
	...
	for (size_t i = 0; i < cs.size(); ++i) {
		const char *name = cs[i].c_str();
		try {
			data = convert_to<std::string>((cs[i] + "//IGNORE").c_str(),
			       data, rawsize(data), name);
		} catch (const unknown_charset_exception &) {
			continue;
		}
    ...
    vmime::shared_ptr<vmime::body> vmBody, IMessage* lpMessage, bool bAppendBody)
{
	...
	try {
		...
		auto strUnicodeText = convert_to<std::wstring>(CHARSET_WCHAR "//IGNORE",
		                      strBuffOut, rawsize(strBuffOut), mime_charset.getName().c_str());
	...
	try {
		...
		if (bAppendBody && m_mailState.bodyLevel == BODY_HTML && m_mailState.ulLastCP && sCodepage.Value.ul != m_mailState.ulLastCP) {
			...
			if(m_mailState.ulLastCP != 65001) {
				...
				if (hr != hrSuccess)
					return hr;
				strCurrentHTML = convert_to<utf8string>(strCurrentHTML, rawsize(strCurrentHTML), lpszCharset).m_str;
			...
			if (sCodepage.Value.ul != 65001)
				...
				strHTML = convert_to<utf8string>(strHTML, rawsize(strHTML), mime_charset.getName().c_str()).m_str;
		...
		if (vmime::dynamicCast<vmime::mediaType>(vmHeader->ContentType()->getValue())->getSubType() ==
			...
			auto strwBody = convert_to<std::wstring>(CHARSET_WCHAR "//IGNORE",
			                strHTML, rawsize(strHTML), mime_charset.getName().c_str());
			strHTML = "<pre>";
