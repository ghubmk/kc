		...
		try {
				...
				lpVMMessageBuilder->getTextPart()->getType() == vmime::mediaType(vmime::mediaTypes::TEXT, vmime::mediaTypes::TEXT_HTML))
			{
				...
				vmMapiAttach = vmime::make_shared<mapiAttachment>(vmime::make_shared<vmime::streamContentHandler>(inputDataStream, 0),
				               ...
				               vmime::word(convert_to<std::string>(m_strCharset.c_str(), szFilename, rawsize(szFilename), CHARSET_WCHAR), m_vmCharset));
...
HRESULT MAPIToVMIME::parseMimeTypeFromFilename(std::wstring strFilename, vmime::mediaType *lpMT, bool *lpbSendBinary)
{
	...
	auto strExt = convert_to<std::string>(m_strCharset.c_str(),
	              strFilename, rawsize(strFilename), CHARSET_WCHAR);
    ...
    vmime::shared_ptr<vmime::message> *lpvmMessage)
{
	...
	try {
		...
		if (hr == hrSuccess) {
			...
			strMDNText = convert_to<std::string>(m_strCharset.c_str(),
			             strBuffer, rawsize(strBuffer), CHARSET_WCHAR);
		}
    ...
    vmime::shared_ptr<vmime::address> &vmMailboxNew)
{
	...
	if (strName.empty() && !strEmail.empty()) {
		...
		// email with fullname
		vmMailboxNew = vmime::make_shared<vmime::mailbox>(getVmimeTextFromWide(strName),
		               convert_to<std::string>(strEmail));
		return hrSuccess;
...
HRESULT MAPIToVMIME::handleTextparts(IMessage* lpMessage, vmime::messageBuilder *lpVMMessageBuilder, eBestBody *bestBody) {
	...
	*bestBody = plaintext;
	...
	try {
		...
		strBodyConverted = convert_to<std::string>(m_strCharset.c_str(),
		                   strBody, rawsize(strBody), CHARSET_WCHAR);
		...
		if (!strHTMLOut.empty()) {
			if (m_vmCharset.getName() != m_strHTMLCharset)
				...
				strHTMLOut = convert_to<std::string>(m_vmCharset.getName().c_str(),
				             strHTMLOut, rawsize(strHTMLOut), m_strHTMLCharset.c_str());
...
vmime::text MAPIToVMIME::getVmimeTextFromWide(const wchar_t *lpszwInput)
{
	try {
		...
		return vmime::text(vmime::word(convert_to<std::string>("US-ASCII//NOIGNORE",
		       lpszwInput, rawsize(lpszwInput), CHARSET_WCHAR), vmime::charsets::US_ASCII));
	} catch (const convert_exception &) {
	}
	return vmime::text(vmime::word(convert_to<std::string>(m_strCharset.c_str(),
